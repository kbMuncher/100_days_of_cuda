...
int main() {
  ...
  dim3 threads(256);
  int segment_size = COARSE_FACTOR * 2 * threads.x;
  dim3 blocks((SIZE + segment_size - 1) / segment_size);

  cudaMemcpy(arr_d, arr, SIZE * sizeof(int), cudaMemcpyHostToDevice);

  sum_reduction_kernel_segmented_coarsed<<<blocks, threads>>>(arr_d, output_d);
